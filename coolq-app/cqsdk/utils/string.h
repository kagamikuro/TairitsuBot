...
namespace cq::utils {
    ...
    inline std::string ws2s(const std::wstring &ws) { return std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t>().to_bytes(ws); }

    inline std::wstring s2ws(const std::string &s) { return std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t>().from_bytes(s); }

    inline std::string ansi(const std::string &s) { return string_encode(s, Encoding::ANSI); }
} // namespace cq::utils
