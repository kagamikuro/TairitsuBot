...
private:
    bool is_active = true;
    Dictionary<Dictionary<std::pair<std::chrono::system_clock::time_point, int>>> group_member_timer;
    Dictionary<std::pair<std::chrono::system_clock::time_point, int>> group_timer;
    Dictionary<std::pair<std::chrono::system_clock::time_point, int>> user_timer;
...
public:
    explicit MessageReceived(const int group_member = 0, const int group = 0, const int user = 0) :
        group_member_cool_down(group_member), group_cool_down(group), user_cool_down(user) {}
