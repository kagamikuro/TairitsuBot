...
Result BanUnbanMembers::check_ban(const cq::Target& current_target, const std::string& message) const
{
    ...
    bool result = std::regex_match(message, ban_regex);
    if (result)
    {
        const int64_t member_id = utility::get_first_id_in_string(message);
        if (user_id != utility::developer_id && !utility::is_admin(group_id, user_id) && utility::is_admin(group_id, utility::self_id) && user_id != member_id)
    ...
    result = std::regex_match(message, ban_long_regex);
    if (result)
    {
        const int64_t member_id = utility::get_first_id_in_string(message);
        if (user_id != utility::developer_id && !utility::is_admin(group_id, user_id) && utility::is_admin(group_id, utility::self_id) && user_id != member_id)
            ...
            utility::ban_group_member(group_id, member_id, 21600, true, u8"怎么，你想让我分享给你我曾感受到过的痛苦？"
                u8"那正好，请细细体会吧。");
        return Result{ true, true };
    }
    return Result{};
...
Result BanUnbanMembers::check_unban(const cq::Target& current_target, const std::string& message) const
{
    const int64_t group_id = *current_target.group_id;
    const int64_t user_id = *current_target.user_id;
    const bool result = std::regex_match(message, unban_regex);
    if (!result) return Result{};
    const int64_t member_id = utility::get_first_id_in_string(message);
    if (user_id != utility::developer_id && !utility::is_admin(group_id, user_id) &&
        utility::is_admin(group_id, utility::self_id) && user_id != member_id)
    {
