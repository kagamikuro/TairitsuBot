...
bool UnoGame::is_required_card(const UnoCard& card) const
{
...
UnoGame::UnoGame(const std::vector<std::pair<int64_t, std::string>>& players)
{
...
void UnoGame::set_required_card(const UnoCard& card)
{
...
int UnoGame::get_current_player_count() const
{
    int player_count = 0;
...
bool UnoGame::can_play(const UnoCard& card) const
{
...
bool UnoGame::has_card(const UnoCard& card) const
{
    if (card.number == UnoCard::None) return true;
    UnoCard wild = card;
...
std::string UnoGame::draw_cards(const int count, const int player)
{
...
std::string UnoGame::play(const UnoCard& card, const bool uno)
{
