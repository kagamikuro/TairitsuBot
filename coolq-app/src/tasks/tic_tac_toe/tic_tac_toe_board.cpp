...
void TicTacToeBoard::draw_small(Image& image, const int row, const int column, const Spot spot) const
{
    ...
    switch (spot)
    {
    case Spot::X: image.draw_image(x, y, x_small_); return;
    case Spot::Blank: return;
    case Spot::O: image.draw_image(x, y, o_small_); return;
    default: throw std::exception("Enum out of range");
    }
}
...
void TicTacToeBoard::draw_large(Image& image, const int row, const int column, const Spot spot) const
{
    ...
    switch (spot)
    {
    case Spot::X: image.draw_image(x, y, x_large_); return;
    case Spot::Blank: return;
    case Spot::O: image.draw_image(x, y, o_large_); return;
    case Spot::Draw: image.draw_image(x, y, draw_); break;
    default: throw std::exception("Enum out of range");
    }
}
